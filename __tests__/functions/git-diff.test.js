...
test('executes gitDiff', async () => {
  ...
  expect(infoMock).toHaveBeenCalledWith('🧮 total detected files changed (raw diff): 5')
  expect(infoMock).toHaveBeenCalledWith('🧮 total detected files changed (json diff): 5')
})
...
test('executes gitDiff with binary files', async () => {
  ...
  expect(infoMock).toHaveBeenCalledWith('🧮 total detected files changed (raw diff): 7')
  expect(infoMock).toHaveBeenCalledWith('🧮 total detected files changed (json diff): 7')
})
...
test('executes gitDiff with binary files and --binary flag and breaks (bug test)', async () => {
  ...
  expect(infoMock).toHaveBeenCalledWith('🧮 total detected files changed (raw diff): 7')
  expect(infoMock).toHaveBeenCalledWith('🧮 total detected files changed (json diff): 4')
})
